...
connected.
    ...
    curl http://localhost:35731/changed?files=style.css

    # notify using a longer path
    curl http://localhost:35731/changed?files=js/app.js

    # notify multiple changes, comma or space delimited
    curl http://localhost:35731/changed?files=index.html,style.css,docs/docco.css

Or you can bulk the information into a POST request, with body as a JSON array of files.

    curl -X POST http://localhost:35731/changed -d '{ "files": ["style.css", "app.js"] }'

As for the livereload client, you need to install the browser extension:
http://feedback.livereload.com/knowledgebase/articles/86242-how-do-i-install-and-use-the-browser-extensions-
(**note**: you need to listen on port 35731 to be able to use with your
brower extension)
...
This package exposes a `bin` you can decide to install globally, but it's not recommended.
    ...
    Options:
      ...
      -p, --port        - Port to listen on (default: 35731)
...
app.use(express.query())
  ...
  .listen(35731, function() {
    console.log('Listening on %d', 35731);
  })
```

The port you listen on is important, and tinylr should **always** listen on
the LiveReload standard one: `35731`. Otherwise, you won't be able to rely
...
$(CSS_DIR): $(CSS_FILES)
  @echo CSS files changed: $?
    @touch $@
  curl -X POST http://localhost:35731/changed -d '{ "files": "$?" }'
